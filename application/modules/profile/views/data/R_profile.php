				...
				<div class='card-body p-0 pl-30 pr-30 pt-15'>
					<div class='row mb-10 pl-15 pr-15'>
						<div class='media'>
							...
							<div class='media'>
								<a href='<?php if ((bool)$book['is_pdf'] == 1) { echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true).'/pdf'); }else{ echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true)); } ?>'><img alt='<?php echo $book['title_book']; ?>' class='w-100 imgcover cover_image' src='<?php echo $book['cover_url']; ?>'></a>
							</div><a class='segment' data-href='<?php echo $book['book_id']; ?>' href='<?php if ((bool)$book['is_pdf'] == 1) { echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true).'/pdf'); }else{ echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true)); } ?>'>
							...
							<div class='bg-white card-footer text-muted pr-30 pl-30' style='font-size: 0.8em;font-weight: bold;'>
								...
								<div>
									<a data-id='<?php echo $book['book_id']; ?>' href='javascript:void(0);' class="<?php if($book['is_like'] == false){ echo 'like'; }else{ echo 'unlike'; } ?>"><img class='mr-20 loveicon' src='<?php if($book['is_like'] == false){ echo base_url('public/img/assets/icon_love.svg'); }else{ echo base_url('public/img/assets/love_active.svg'); } ?>' width='27'></a> <a href='<?php if ((bool)$book['is_pdf'] == 1) { echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true).'/pdf'); }else{ echo site_url('book/'.$book['book_id'].'-'.url_title($book['title_book'], 'dash', true)); } ?>#comments'><img class='mr-10' src='<?php echo base_url('public/img/assets/icon_comment.svg'); ?>' width='25'></a>
								</div>
							</div>
						</div>
						<?php } }else { } ?>
