                    ...
                    if (Object.keys(data.user).length > 0) {
                        ...
                        if (bak.length >= 1) {
                            $.each(data.user, function (valkey, valid) {
                                ...
                                userhtml += "<div class='card mr-10 pull-left' style='width:15%;'> <div class='container'> <div class='row'> <div class='col-12'> <div class='text-center p-10'> <img src='" + prof_picts + "' width='50' height='50' class='rounded-circle'> <p class='nametitled'><a href='"+base_url+"penulis/"+convertToSlug(valid.fullname)+"-"+valid.user_id+"'>" + valid.fullname + "</a></p> </div> </div> </div> <div class='row'> <div class='col-6 text-center rborder'> <p style='display: inline-flex;'><img src='public/img/icon-tab/book.svg' width='25'> <span>" + valid.book_made + "</span></p> <h6>Buku</h6> </div> <div class='col-6 text-center'> <p style='display: inline-flex;'><img src='public/img/icon-tab/followers.svg' width='25'> <span>" + valid.followers + "</span></p> <h6>Teman</h6> </div> </div> <br> <div class='row'> <div class='col-12 text-center'> <button class='btnfollow-f "+follow+"' data-follow='" + valid.user_id + "'><img src='public/img/icon-tab/add_follow.svg' width='30'> <span class='txtfollow'>"+txtfol+"</span></button> </div> </div> <br> </div> </div>";
                            });
                        }
                        $('.loader').hide();
            ...
            .done(function(data) {
                if (search.length >=3) {
                    ...
                    $.each(data['books'], function(index, val) {
                        ...
                        var urls = base_url+'penulis/'+convertToSlug(val.author_name)+'-'+ val.author_id+'/'+convertToSlug(val.title_book)+'-'+val.book_id;
