...
function detailEPUB() {
    var id = $('#iaidubi').val();
    $.ajax({
        url: base_url + 'detailBooks',
        type: 'POST',
        dataType: 'json',
        data: {book_id: id, csrf_test_name: csrf_value}
    }).done(function (data) {
        if (data.c == 200) {
            yourGlobalVariable += data.dat.u;
            ePubViewer.doBook(yourGlobalVariable);
        }
    }).fail(function () {
    }).always(function () {
    });
...
App.prototype.doBook = function (url, opts) {
    ...
    try {
        this.state.book = ePub(atob(yourGlobalVariable), opts);
    ...
    // console.log('ready', this.state.book.ready);
    // console.log('loaded', this.state.book.loaded);
...
App.prototype.loadSettingsFromStorage = function () {
    ["theme", "fonts", "font-size", "line-spacing", "margin"].forEach(container => this.restoreChipActive(container));
};
...
App.prototype.onNavigationLoaded = function (nav) {
    ...
    let handleItems = (items, indent) => {
        ...
        a.innerHTML = "<h2 class='h6'>"+`${"&nbsp;".repeat(indent*4)}${item.label.trim()}`+"</h2>";
